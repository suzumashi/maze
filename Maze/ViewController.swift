...
class ViewController: UIViewController {
    ...
    var playerMottionManager: CMMotionManager!
    ...
    override func viewDidLoad() {
        ...
        for y in 0..<maze.count {
            for x in 0..<maze[y].count{
                switch maze[y][x] {
                case 1:
                    let wallView = createView(x: x, y: y, width: cellWidth, height: cellHeight, offseX: cellOffsetX, offsetY: cellOffsetY)
                    wallView.backgroundColor = UIColor.black
                    view.addSubview(wallView)
                    wallRectArray.append(wallView.frame)
                case 2:
                    startView = createView(x: x, y: y, width: cellWidth, height: cellHeight, offseX: cellOffsetX, offsetY: cellOffsetY)
                    startView.backgroundColor = UIColor.green
                    view.addSubview(startView)
                case 3:
                    goalView = createView(x: x, y: y, width: cellWidth, height: cellHeight, offseX: cellOffsetX, offsetY: cellOffsetY)
        ...
        playerMottionManager = CMMotionManager()
        playerMottionManager.accelerometerUpdateInterval = 0.02
        
        startAccelerometer()
    }
    
    func createView(x: Int, y: Int, width: CGFloat, height: CGFloat, offseX: CGFloat, offsetY: CGFloat) -> UIView {
        let rect = CGRect(x: 0, y: 0, width: width, height: height)
        let view = UIView(frame: rect)
        
        let center = CGPoint(x: offseX + width * CGFloat(x), y: offsetY + height * CGFloat(y))
    ...
    func startAccelerometer() {
        ...
        playerMottionManager.startAccelerometerUpdates(to: OperationQueue.main, withHandler: handler)
    }
        
        func gameCheck(result: String, message: String){
            if playerMottionManager.isAccelerometerActive {
                playerMottionManager.stopGyroUpdates()
            }
